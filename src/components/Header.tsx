...
export function Header() {
  ...
  return (
    <header className="sticky top-0 z-50 w-full bg-white/80 backdrop-blur-xl border-b border-gray-200/50 shadow-lg">
      <div className="container mx-auto px-4 sm:px-6 lg:px-8">
        <div className="hidden lg:flex items-center justify-center py-2.5 border-b border-gray-100">
          <div className="flex items-center gap-3 text-xs text-gray-600 tracking-wider uppercase">
            <Star className="w-3.5 h-3.5 text-[#F29038] fill-current" />
            <span className="font-light">Premier Fertility Specialist in Mumbai</span>
            <Star className="w-3.5 h-3.5 text-[#F29038] fill-current" />
          </div>
        </div>
        ...
        <div className="flex h-20 items-center justify-between">
          ...
          <nav className="hidden lg:flex items-center gap-6 xl:gap-8 flex-1 justify-center px-8">
            {navLinks.map((link) => (
              <Link
                key={link.href}
                href={link.href}
                className="text-sm font-light text-gray-700 hover:text-[#66257B] transition-colors relative group whitespace-nowrap"
              >
                {link.label}
                <span className="absolute -bottom-1 left-0 w-0 h-0.5 bg-[#66257B] group-hover:w-full transition-all duration-300"></span>
              </Link>
            ))}
          </nav>
          ...
          <div className="hidden lg:flex items-center gap-4 flex-shrink-0">
            ...
            <Button
              asChild
              className="bg-gradient-to-r from-[#66257B] to-[#7a2e94] hover:from-[#7a2e94] hover:to-[#66257B] text-white px-5 py-2 shadow-lg hover:shadow-xl transition-all whitespace-nowrap"
            >
              <Link href="/contact">
            ...
            <nav className="flex flex-col space-y-3">
              {navLinks.map((link) => (
                <Link
                  key={link.href}
                  href={link.href}
                  className="text-sm text-gray-700 hover:text-[#66257B] transition-colors py-2"
              ...
              <Button asChild className="bg-gradient-to-r from-[#66257B] to-[#7a2e94] text-white w-full mt-3">
